...
public slots:
    ...
    void onCreateClientPlaneExecuted() override;
	void onFindClientSearchExecuted() override;
    void onFindClientViewClientsExecuted() override;
	void onEditClientSaveExecuted() override;
	void onEditClientDeleteExecuted() override;
	void onRssRefreshExecuted() override;

signals:
     void viewPlanesExecuted();
     void fillAll();  /// Plane and seat must be selected
     void checkLocation(); /// Locations Must no be the same
